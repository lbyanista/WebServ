...
int ListingFolderContent(char *path) {
...
{
//    std::cout << ListingFolderContent(av[1]);
    std::string str;

    std::string ext = "Hello.*3gpp";
    ...
    std::map<std::string, std::string> extension(old_arr, old_arr + n);;


    std::vector<std::string> key, value;
    for(std::map<std::string,std::string>::iterator it = extension.begin(); it != extension.end(); ++it) {
    ...
    // while (std::map<std::string,std::string>::iterator it = extension.begin(); it != extension.end(); ++it)
    // {
    //     std::string path = av[1];
    //     if(path.substr(path.find_last_of(.) + 1) == it->first)
    //         std::cout << it->second;
    // }
